...
function CommunityOverview({ resourceType } : CommunityOverviewProps) {
    ...
    return (
        <>
            <div className={`flex min-h-full flex-col justify-center items-center py-12 ${resourceType == 'Topic' ? 'px-4 sm:px-6 lg:px-8' : 'px-4 xl:px-6'}`}>
                <div className={`w-full ${resourceType == 'Topic' ? 'max-w-4xl' : 'max-w-screen-2xl sm:pl-10'}`}>
                    ...
                    {!postsLoading && (!topicsLoading || resourceType != 'Topic') ? (postsCollection?.docs?.length > 0 ?
                        <>
