...
import type SearchResultHit from './types/SearchResultHit';
...
const SearchResults = () => {
    ...
    const [results, setResults] = useState<SearchResultHit[] | null>(null);
        ...
        <>
            <div className="flex min-h-full flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8">
                <div className="min-w-[75%]">
                                    ...
                                    return (
                                        ...
                                        >
                                            <p className="text-sm text-gray-500 mb-1 flex">
                                                <ChatBubbleLeftEllipsisIcon
                                                    className="fill-gos-green-light mr-1.5 h-5 w-5 flex-shrink-0 text-gray-400"
                                                    aria-hidden="true"
                                                />
                                                Post - {result.authorUsername}{' '}
                                                {!!result.createdAt &&
                                                    '- ' +
                                                        dayjs(result.createdAt).calendar(null, {
                                                            sameElse: 'lll',
                                                        })}
                                            </p>

                                            <h4 className="text-2xl mt-4">{result.title}</h4>
                                        </Link>
                                    );
                                case 'action':
                                    return (
                    ...
                    {results !== null ? (
                        <Pagination totalCount={totalMatches} pageSize={PAGE_SIZE} />
                    ) : (
                        <Loader />
                    )}
                </div>
